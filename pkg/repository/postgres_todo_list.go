...
func (r *TodoListPostgres) GetAll(userId int) ([]todo.TodoList, error) {
	...
	query := fmt.Sprintf("SELECT tl.id, tl.title, tl.description FROM %s tl INNER JOIN %s ul on  tl.id = ul.list_id AND ul.user_id = $1", todoListTable, usersListTable)
...
func (r *TodoListPostgres) GetById(userId int, listId int) (todo.TodoList, error) {
	...
	query := fmt.Sprintf("SELECT tl.id, tl.title, tl.description FROM %s tl INNER JOIN %s ul on  tl.id = ul.list_id AND ul.user_id = $1 AND tl.id = $2", todoListTable, usersListTable)
